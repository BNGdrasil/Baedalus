...
3. 이미지를 내려받고, 교체 직전 컨테이너의 이미지 ID를 `rollback/<컨테이너>:<UTC 시각>` 태그로
   남깁니다. 태그가 아니라 이미지 ID를 대상으로 삼기 때문에, 같은 태그가 registry에서 새 이미지로
   덮인 뒤에도 이전 layer를 되찾을 수 있습니다.
4. `/opt/bnbong/.env`의 `AUTH_SERVER_IMAGE` 또는 `GATEWAY_IMAGE` 줄만 새 값으로 바꿉니다. 받은
   참조를 가공하지 않고 그대로 기록하므로, digest로 호출하면 `.env`에도 digest가 남습니다. 나머지
   줄과 파일 권한은 그대로 유지합니다.
5. `docker compose up -d --no-deps --no-build <서비스>`로 해당 서비스만 교체합니다.
6. health를 확인합니다. 두 서비스 모두 `/health`가 200을 돌려주어야 하고, Gateway는 `/ready`까지
   200이어야 합니다. `/ready`는 데이터베이스 연결과 서비스 등록부를 함께 확인하므로, 이 검사를
   생략하면 프로세스만 살아 있고 의존 자원이 끊긴 상태를 성공으로 오인합니다.
7. 확인에 실패하면 `.env`를 이전 내용으로 되돌리고 이전 이미지로 다시 기동한 다음, 0이 아닌 종료
   코드로 끝냅니다. 워크플로도 이 종료 코드를 그대로 받아 실패로 기록됩니다.
8. 성공하면 시각과 서비스 이름과 이미지 참조와 digest를 `/opt/bnbong/releases.log`에 한 줄로
...
이미지로 컨테이너를 다시 기동합니다. 이때 워크플로는 실패로 끝나고, 로그의 마지막 부분에 되돌아간
지점과 확인 명령이 출력됩니다. 운영자는 서비스가 이전 상태로 돌아왔는지만 확인하면 됩니다.
